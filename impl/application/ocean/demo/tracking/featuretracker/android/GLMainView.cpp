...
void GLMainView::threadRun()
{
	...
	while (shouldThreadStop() == false)
	{
		...
		if (featureTrackerWrapper_.trackNewFrame(resultingTrackerFrame, resultingTrackerPerformance) && resultingTrackerFrame.isValid())
		{
			...
			// Log::info() << "Aman 85 + " << resultingTrackerPerformance * 1000.0 << "ms + " << resultingTrackerFrame.isValid();
...
jboolean Java_com_flam_fit_app_demo_tracking_featuretracker_android_FeatureTrackerActivity_initializeFeatureTracker(JNIEnv* env, jobject javaThis, jstring inputMedium, jstring pattern, jstring resolution)
{
...
jstring Java_com_flam_fit_app_demo_tracking_featuretracker_android_FeatureTrackerActivity_boundingBoxEdges(JNIEnv* env, jobject javaThis) {
	...
	std::string resultStr = featureTrackerWrapper.boundingBoxEdges();
